...
void processCommand (Context_t * context, int argc, char* argv[])
{
    ...
    if (argc > 1)
    {
		...
		while (i < argc)
		{
			...
			else if ((strcmp(argv[i], "-nf") == 0) || (strcmp(argv[i], "--setOnOff") == 0))
	        {
	        	if (i + 1 <= argc)
				{
					int OnOff;
					...
					/* set display OnOff */
					if (((Model_t*)context->m)->SetOnOff)
						((Model_t*)context->m)->SetOnOff(context, OnOff);
		        }
				i += 1;
		    }
			else if ((strcmp(argv[i], "-gmt") == 0) || (strcmp(argv[i], "--setGmt") == 0))
	        {
	        	if (i + 1 <= argc)
				{
					int Gmt;
					...
					/* set display Gmt */
					if (((Model_t*)context->m)->SetGmt)
						((Model_t*)context->m)->SetGmt(context, Gmt);
		        }
				i += 1;
		    }
