...
static int setTimer(Context_t* context, time_t* theGMTTime)
{
   ...
   time_t                  wakeupTime;
   struct tm               *ts;
   tHL101Private* private = (tHL101Private*)
...
static int setOnOff(Context_t* context, int onoff)
{
   struct proton_ioctl_data vData;
   ...
   vData.u.onoff.level = onoff;

   printf("%d\n", context->fd);
   if (ioctl(context->fd, VFDONOFF, &vData) < 0)
   {
      perror("setonoff: ");
      return -1;
   }
...
static int setGmt(Context_t* context, int gmt)
{
   struct proton_ioctl_data vData;
   ...
   vData.u.gmt.level = gmt;

   printf("%d\n", context->fd);
   if (ioctl(context->fd, VFDGMT, &vData) < 0)
   {
      perror("setgmt: ");
      return -1;
   }
