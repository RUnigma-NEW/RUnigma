...
static enum stb0899_status stb0899_check_data(struct stb0899_state *state)
{
	...
	int lock, index = 0, dataTime, loop;
	u8 reg;

	/* check if already locked, no need to restart derot */
	reg = stb0899_read_reg(state, STB0899_VSTATUS);
	lock = STB0899_GETFIELD(VSTATUS_LOCKEDVIT, reg);
	if (lock) {     /* DATA LOCK indicator  */
		internal->status = DATAOK;
		dprintk(state->verbose, FE_DEBUG, 1, "-----------------> EARLY DATA OK !");
		return internal->status;
	}
	else if ( !lock )
	{
...
enum stb0899_status stb0899_dvbs_algo(struct stb0899_state *state)
{
	...
	s32 betaTab[5][4] = {
		...
		{ 37,  34,  32,  31 }, /* QPSK 2/3	*/
		{ 37,  35,  33,  32 }, /* QPSK 3/4	*/
		{ 37,  35,  33,	 33 }, /* QPSK 5/6	*/
		{ 37,  35,  33,	 33 }  /* QPSK 7/8	*/
	};
